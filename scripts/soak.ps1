...
# 客户端:Web×2 + Android 模拟器 + Windows Release(AUTO_JOIN 常驻挂机端)
param([int]$Expect = 4)
$logFile = "D:\Projects\Lares\soak.log"
"$(Get-Date -Format 'yyyy-MM-dd HH:mm:ss') soak start, expect home=$Expect" | Out-File $logFile -Append
$failStreak = 0
while ($true) {
    try {
        $h = Invoke-RestMethod http://localhost:8787/health -TimeoutSec 5
        $count = $h.circles.home
        $status = if ($count -eq $Expect) { 'OK' } else { "DEGRADED($count/$Expect)" }
        if ($count -eq $Expect) { $failStreak = 0 } else { $failStreak++ }
    } catch {
